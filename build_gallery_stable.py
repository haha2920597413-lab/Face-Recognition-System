...
# 降低 OpenCV 在 Windows 长时间批处理时的后台线程/显存/OpenCL 风险
cv2.setNumThreads(0)
...
def _try_create_session(model_path: Path, providers: List[str]) -> ort.InferenceSession:
    ...
    稳定性增强：
        1. 限制 ONNX Runtime 为单线程顺序执行。
        2. 关闭 CPU 内存 arena 和 memory pattern。
        3. 降低图优化级别。

    这些设置会牺牲少量速度，但能显著降低 Windows 下长时间批量推理时
    python.exe 原生层访问冲突的概率。
    """
...
def extract_embedding(
    ...
    稳定性增强：
        显式 copy ONNX Runtime 输出，避免长期循环中引用 ORT 内部输出缓冲区。
    """
...
def save_multi_template_gallery(
    ...
    if features_array.ndim != 2:
        raise ValueError(f"features_array 维度异常: {features_array.shape}")

    if features_array.shape[0] != len(labels):
        raise ValueError(
            f"待保存的 gallery 数据不一致: "
            f"features={features_array.shape[0]}, labels={len(labels)}"
        )
...
def build_gallery(config_path: str) -> None:
    ...
    save_every_n = int(gallery_cfg.get("save_every_n", 20))
    if save_every_n <= 0:
        save_every_n = 20
    ...
    print(f"[INFO] save_every_n: {save_every_n}")
    print("=" * 80)
    ...
    for idx, identity_dir in enumerate(identity_dirs, start=1):
        ...
        try:
            ...
            # 断点续跑关键机制：
            # 不再每成功处理一个身份就完整保存一次 gallery，避免后期反复 np.asarray + 写盘
            # 带来的内存和 I/O 压力。默认每 20 个新增身份保存一次，程序结束时仍会最终保存。
            if num_success_new % save_every_n == 0:
    ...
    if len(gallery_features) > 0:
        final_features = np.asarray(gallery_features, dtype=np.float32)
