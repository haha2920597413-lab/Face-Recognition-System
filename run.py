...
def select_main_face(
    ...
    规则：
        1. 只选择 status == "Known" 的人脸；
        2. Unknown / Failed 不参与播报；
        3. score 必须 >= min_score；
        4. 当前版本按 score 最高选择主人脸。
    """
    candidates: List[Dict[str, Any]] = []

    for face in face_results:
        status = str(face.get("status", ""))
        if status != "Known":
            continue
        ...
        employee_id = str(face.get("employee_id", ""))
        name = str(face.get("name", ""))

        if employee_id in ["", "Unknown", "Failed"]:
            continue

        if name in ["", "Unknown", "Failed"]:
            continue
    ...
    raise ValueError(f"不支持的主人脸选择策略: {strategy}")
...
class PiperTTSWorker:
    ...
    工作方式：
        主线程把文本放入队列；
        后台线程调用 piper.exe 生成临时 wav；
        播放 wav；
        播放完成后删除临时 wav。

    注意：
        temp_audio_path 只是临时文件，每次播报都会覆盖，播完删除。
    """
    ...
    def __init__(
        ...
        delete_temp_audio_after_play: bool = True,
        print_announce: bool = True,
    ):
        self.project_root = project_root
        ...
        self.delete_temp_audio_after_play = bool(delete_temp_audio_after_play)
        self.print_announce = bool(print_announce)
    ...
    def _check_files(self) -> None:
        if not self.piper_exe.exists():
            raise FileNotFoundError(f"Piper 可执行文件不存在: {self.piper_exe}")

        if not self.model_path.exists():
            raise FileNotFoundError(f"Piper 模型不存在: {self.model_path}")

        if self.config_path is not None and not self.config_path.exists():
            raise FileNotFoundError(f"Piper 配置文件不存在: {self.config_path}")

        self.temp_audio_path.parent.mkdir(parents=True, exist_ok=True)

    def start(self) -> None:
        self.thread.start()
        print("[INFO] Piper TTS 后台线程已启动。")
    ...
    def enqueue(self, text: str) -> bool:
        ...
        if self.drop_if_queue_full and self.text_queue.full():
            print(f"[VOICE] 队列已满，丢弃播报: {text}")
            return False
    ...
    def _run(self) -> None:
        while not self.stop_event.is_set():
            ...
            try:
                ...
                print(f"[VOICE ERROR] 播报失败: {e}")

    def _speak_once(self, text: str) -> None:
        if self.print_announce:
        ...
        if result.returncode != 0:
            print("[VOICE ERROR] Piper 执行失败。")
            print(f"[VOICE ERROR] command: {' '.join(cmd)}")
            print(f"[VOICE ERROR] stderr: {result.stderr}")
            return

        if not self.temp_audio_path.exists():
            print(f"[VOICE ERROR] Piper 未生成音频文件: {self.temp_audio_path}")
            return

        self._play_wav(self.temp_audio_path)

        if self.delete_temp_audio_after_play:
            self._delete_temp_audio()
    ...
    def _play_wav(self, wav_path: Path) -> None:
        try:
            ...
            print(f"[VOICE ERROR] 播放 wav 失败: {wav_path}, 错误: {e}")

    def _delete_temp_audio(self) -> None:
        try:
...
class VoiceAnnounceManager:
    ...
    规则：
        1. 只在检测帧调用；
        2. 过滤 Unknown / Failed；
        3. 从 Known 人脸中选择 score 最高者作为主人脸；
        4. 同一个 employee_id cooldown_seconds 秒内不重复播报；
        5. 文本模板为：{name}老师您好；
        6. 放入 PiperTTSWorker 队列，由后台线程播报。
    """
    ...
    def __init__(
        ...
        text_template: str = "{name}老师您好",
        main_face_strategy: str = "score",
        print_announce: bool = True,
    ):
        ...
        self.main_face_strategy = str(main_face_strategy)
        self.print_announce = bool(print_announce)

        self.last_announce_time: Dict[str, float] = {}

    def handle_frame_results(
        self,
        face_results: List[Dict[str, Any]],
        frame_id: int,
        timestamp: str,
        ...
        main_face = select_main_face(
            ...
            strategy=self.main_face_strategy,
        )

        if main_face is None:
            return

        employee_id = str(main_face.get("employee_id", ""))
        name = str(main_face.get("name", ""))
        score = float(main_face.get("score", 0.0))

        now = time.time()
        last_time = self.last_announce_time.get(employee_id, -1e18)

        if now - last_time < self.cooldown_seconds:
            if self.print_announce:
                remain = self.cooldown_seconds - (now - last_time)
                print(
                    f"[VOICE] 跳过播报: {name}, "
        ...
        ok = self.tts_worker.enqueue(text)

        if ok:
            self.last_announce_time[employee_id] = now

            if self.print_announce:
                print(
                    f"[VOICE] 加入播报队列: {text}, "
                    f"employee_id={employee_id}, score={score:.4f}"
                )
...
def run_video_face_recognition(config_path: str) -> None:
    ...
    voice_enable = bool(voice_cfg.get("enable", False))

    if voice_enable:
        tts_engine = str(voice_cfg.get("tts_engine", "piper")).lower().strip()

        if tts_engine != "piper":
            raise ValueError(f"当前只支持 Piper TTS，不支持: {tts_engine}")

        piper_exe = resolve_path(project_root, voice_cfg["piper_exe"])
        piper_model_path = resolve_path(project_root, voice_cfg["model_path"])

        piper_config_value = voice_cfg.get("config_path", None)
        if piper_config_value is None:
            piper_config_path = None
        else:
            piper_config_path = resolve_path(project_root, piper_config_value)

        temp_audio_path = resolve_path(
            project_root,
            voice_cfg.get("temp_audio_path", "temp/tts_temp.wav"),
        )
        ...
        tts_worker = PiperTTSWorker(
            ...
            queue_max_size=int(voice_cfg.get("queue_max_size", 5)),
            drop_if_queue_full=bool(voice_cfg.get("drop_if_queue_full", True)),
            delete_temp_audio_after_play=bool(
                voice_cfg.get("delete_temp_audio_after_play", True)
            ),
            print_announce=bool(voice_cfg.get("print_announce", True)),
        )

        tts_worker.start()

        voice_manager = VoiceAnnounceManager(
            tts_worker=tts_worker,
            min_score=float(voice_cfg.get("min_score", threshold)),
            cooldown_seconds=float(voice_cfg.get("cooldown_seconds", 180)),
            text_template=str(voice_cfg.get("text_template", "{name}老师您好")),
            main_face_strategy=str(voice_cfg.get("main_face_strategy", "score")),
            print_announce=bool(voice_cfg.get("print_announce", True)),
        )

        print("[INFO] 语音播报模块已启用。")
        ...
        while True:
            ...
            if should_process:
                ...
                if voice_manager is not None:
                    voice_manager.handle_frame_results(
                        face_results=face_results,
                        frame_id=frame_id,
                        timestamp=timestamp,
                    )
